...
Constant::Constant(double amplitude, std::string graph_name, double sample_freq, double x_min, double x_max, float line_colour[4], int line_type, std::string label_x, std::string label_y)
    : Graph(GraphType::Constant, graph_name, sample_freq, x_min, x_max, line_colour, line_type, label_x, label_y)
{
...
{
    m_graph_data = std::vector<double>(static_cast<size_t>((m_base_arg._x_max - m_base_arg._x_min) * m_base_arg._sample_frequency + 1), m_unique_arg._amplitude);
    saveDataToFile();
}
...
void Constant::renderImGuiEditGraph()
{
    ...
    ImGui::BeginChild(("Constant##list" + m_base_arg._graph_name).c_str(), ImVec2(windowSize.x - 500, windowSize.y - 2), true, ImGuiWindowFlags_NoMove);
